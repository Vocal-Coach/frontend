...
export default function PracticePage({ params }: PracticePageProps) {
  ...
  const handlePitchDetected = (frequency: number, audioData: Float32Array) => {
    ...
    if (animationState.isPlaying && levelData.scale) {
      ...
      const currentNoteIndex = Math.floor((currentTime - startTimeRef.current) / 1000 / beatDuration);
  ...
  return (
    ...
    >
      ...
      {/* 점수 표시 - 점수 플래시 효과 적용 */}
      <div className={`transition-all duration-300 mt-1 mb-1 ${scoreFlash ? 'scale-110 text-green-500' : ''}`}>
      ...
      <div className="flex justify-center mt-2 space-x-2">
        <button 
          ...
          className={`flex items-center px-2 py-0.5 rounded-md text-xs ${isMicActive ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'}`}
        >
          {isMicActive ? (
            <>
